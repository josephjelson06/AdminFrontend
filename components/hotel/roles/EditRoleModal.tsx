...
export function EditRoleModal({ isOpen, onClose, role, onSave }: EditRoleModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsSaving(true);
        const success = await onSave(role.id, {
            name,
            description,
            permissions
        });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="w-full max-w-4xl bg-[#0F172A] border border-white/10 rounded-xl shadow-2xl animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
                ...
                <div className="flex items-center justify-between p-4 border-b border-white/5 shrink-0">
                    <h2 className="text-lg font-semibold text-white">Edit Role: {role.name}</h2>
                    ...
                    <form id="edit-role-form" onSubmit={handleSubmit} className="space-y-4">
                        ...
                        {role.is_system_role && (
                             <div className="p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg text-sm text-blue-400">
                                 Note: This is a system role. Some properties cannot be changed.
                             </div>
                        )}
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className={`w-full pl-9 pr-3 py-2 bg-white/5 border border-white/10 rounded-lg text-white placeholder-white/30 focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 transition-all ${role.is_system_role ? 'opacity-50 cursor-not-allowed' : ''}`}
                                    />
                                </div>
                            </div>
                            <div>
