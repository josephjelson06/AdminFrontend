...
export default function IncidentsPage() {
    ...
    const incomingIncidents = useMemo(() => {
        return incidents
            .filter(inc => inc.assignedToUserId === null && inc.status === 'Reported')
            .sort((a, b) => {
                const priorityA = PRIORITY_ORDER[String(a.priority)] ?? 3;
                const priorityB = PRIORITY_ORDER[String(b.priority)] ?? 3;
                return priorityA - priorityB;
            });
    }, [incidents]);

    const assignedIncidents = useMemo(() => {
        return incidents.filter(inc => inc.assignedToUserId !== null);
    }, [incidents]);

    // Maintenance Staff: Filter only their assigned incidents
    const myIncidents = useMemo(() => {
        return incidents
            .filter(inc => inc.assignedToUserId === user?.id)
            .sort((a, b) => {
                const priorityA = PRIORITY_ORDER[String(a.priority)] ?? 3;
                const priorityB = PRIORITY_ORDER[String(b.priority)] ?? 3;
                return priorityA - priorityB;
